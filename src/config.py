...
class ConfigManager:
    ...
    def __init__(self, base_config_file: Optional[str] = None):
        ...
        self.base_config_file = (
            ...
            if base_config_file
            else self.base_dir / "config.json"
        )
    ...
    def create_lan_client_config(self) -> None:
        """Create a minimal configuration for LAN client mode."""
        project_name = f"lan_client_{int(time.time())}"
        ...
        project_config = {
            **DEFAULT_PROJECT_CONFIG,
            "nbt_file": None,  # No local NBT file needed
